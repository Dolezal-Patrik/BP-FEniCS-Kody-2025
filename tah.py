...
class pointload(UserExpression):
  def eval(self,value,x):
    ...
    if near(x[0],0.0,eps) and near(x[1],-10.0,eps):
      value[0]=-F
      value[1]=0.0
    else:
      value[0]=0.
...
#-----[ definice tenzoru napeti a deformace ]-----
...
plt.legend()
plt.tight_layout()
plt.savefig('průhyb fenics',dpi=600)
...
# --- vypocet maximalni hodnoty slozek napeti ---
sigmax_vals = []
sigmay_vals = []
tauxy_vals = []
...
print("\n--- maximalni hodnoty napeti ---")
print(f"Maximální |σₓ| = {max_sigmax:.2f} MPa")
print(f"Maximální |σ_y| = {max_sigmay:.2f} MPa")
print(f"Maximální |τ_xy| = {max_tauxy:.2f} MPa")
